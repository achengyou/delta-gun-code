...
const fallbackDailyPasswords = [
  { map: "零号大坝", password: "0499", updatedAt: "2026-05-20", source: "manual" },
  { map: "长弓溪谷", password: "0692", updatedAt: "2026-05-20", source: "manual" },
  { map: "巴克什", password: "0662", updatedAt: "2026-05-20", source: "manual" },
  { map: "航天基地", password: "0669", updatedAt: "2026-05-20", source: "manual" },
  { map: "潮汐监狱", password: "0039", updatedAt: "2026-05-20", source: "manual" }
];
...
async function loadDailyPasswords() {
  try {
    const response = await fetch("./data/daily-passwords.json", { cache: "no-store" });
    if (!response.ok) throw new Error("fetch failed");
    const data = await response.json();
    const normalized = Array.isArray(data) ? data.map(normalizeDailyPasswordItem).filter(item => item.map) : [];
    return normalized.length ? normalized : fallbackDailyPasswords.map(normalizeDailyPasswordItem);
  } catch {
    return fallbackDailyPasswords.map(normalizeDailyPasswordItem);
  }
}
...
function getDailyPasswordStatus(item) {
  if (!cleanText(item.updatedAt)) return "待更新";
  return cleanText(item.updatedAt) === getTodayDate() ? "今日已更新" : "可能已过期";
}
...
function renderDailyPasswords() {
  ...
  if (!list.length) {
    grid.innerHTML = `<div class="empty daily-empty" style="display:block;padding:28px 0">暂无今日密码，更新 data/daily-passwords.json 后自动显示。</div>`;
    return;
  }
...
async function init() {
  ...
  state.dailyPasswords = await loadDailyPasswords();
  setupEvents();
  renderAll();
}
